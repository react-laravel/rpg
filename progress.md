...
## 2026-09-20 — Consistent combat units and overhead resource bars

- User requested equally sized monster/player images, HP/MP above the images, and monster rank represented by name colour instead of borders. Both units now use the same 56px mobile / 64px desktop sizes; removed the four-plus-monsters shrinking branch, icon inset padding, rank borders and rank glows.
- Shared numeric resource bars render above each image: HP for monsters, HP/MP for the player. Names remain beneath monsters: ordinary white, elite blue, boss gold. Rank is also available in the button's accessible name/hover text.
- Removed two unused grid rows so five monsters fit a narrow square arena. Casting feedback uses the actual space between units, while damage/regeneration feedback avoids the overhead bars. Existing HP settlement, death retention, shields and effect anchors are preserved.
- Validation: 14 existing combat/anchor/normalization tests, TypeScript, ESLint and production build passed. Eight production-mode browser cases cover 320/390/640/768/1024/1440 widths, 1/3/5 monsters, equal icon dimensions, overhead resources, no permanent monster border/glow, all three name colours, monster detail opening/closing, spell/HP/death feedback, and no overflow or console errors. The square map and player bottom clearance were checked. Screenshots and bundled web-game screenshot/state inspected. Fixtures are isolated; live account data was not changed. Evidence: `output/combat-unit-layout/`.
- Frontend-only change, delivered under existing push authorization; deployment verification is tracked in ignored `output/pixel-rpg/delivery.local.json`.

## 2026-09-22 — Large equipment previews and progression audit

- User reported reaching level 200 too quickly and requested a read-only SSH investigation, then requested larger equipment images. Subsequent choices: target roughly one month of continuous normal auto-combat to level 200; small fixed equipment values with adjacent tier primary-stat increases of 1–2, identical same-name stats across qualities, no random affixes, preserved socketed gems.
- Live PostgreSQL audit used read-only transactions and an explicit UTC session. The main character earned approximately 90 million XP in 24 hours on normal map 41, with zero damage taken; the level-200 threshold is 132.335 million. Attack 454 versus ordinary monster HP 162–243, and defense 686 versus at most 120 monster attack, make late maps trivial. Worker cadence is normal (3–4 seconds), test mode is off and offline rewards were not claimed. Also found difficulty reward applied twice to XP; it does not affect the normal-difficulty observation. Backend rebalance, backups and simulation are in progress in rpg-api; do not mark the overall request complete yet.
- Equipment UI: comparison thumbnails increased from 40px to 56/64px. Both comparison sides and normal/equipped detail thumbnails open an accessible large-image dialog. Images use the existing pixel PNGs directly (no nonexistent `_origin` suffix), preserve crisp scaling, and render above the equipped-item overlay. Escape/close returns to the original comparison or item details and restores focus.
- Frontend validation: TypeScript, ESLint and production build passed; 9 existing inventory/equipment tests plus a preview interaction regression test passed. Browser checks at 320/390/1440px verified both comparison sides, equipped details, correct PNGs, pixel rendering, Escape/close preservation, no overflow or console errors. Bundled web-game client and screenshots inspected. Evidence: `output/item-image-preview/`.
- Publishing this independent frontend fix while the authorized numerical rebalance continues. Final delivery tracking remains in ignored `output/pixel-rpg/delivery.local.json`.
