...
export function VSSwords({
  ...
  return (
          ...
          ? `min-h-11 flex-row gap-1.5 rounded-md border px-2.5 text-xs sm:px-3 sm:text-sm ${
              isCharacterDead
                ...
                : isFighting
                  ...
                  : 'border-primary/40 bg-primary/15 text-amber-700 hover:bg-primary/25 dark:text-amber-300'
            }`
    ...
    >
      ...
      <span
        className={`whitespace-nowrap ${isInline ? 'text-xs sm:text-sm' : 'text-xs sm:text-sm'}`}
      >
        {label}
      </span>
    </button>
