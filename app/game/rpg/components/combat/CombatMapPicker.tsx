  ...
  return (
    ...
    >
      ...
      <PopoverContent
        ...
        aria-label="选择冒险地图"
        className="flex h-[min(32rem,var(--radix-popover-content-available-height))] w-[min(34rem,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-xl p-0 shadow-xl"
      >
        {travelError && (
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          {acts.length > 0 && (
            <div
              className="border-border flex shrink-0 flex-col gap-1 overflow-y-auto border-r p-2"
              aria-label="地图章节"
            >
              {acts.map(act => (
                <button
                  ...
                  aria-label={getActName(act)}
                  onClick={() => setSelectedAct(act)}
                  className={`flex min-h-16 w-9 shrink-0 items-center justify-center rounded-md py-2 text-xs font-medium transition-colors [writing-mode:vertical-rl] ${
                    activeAct === act
